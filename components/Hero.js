  ...
  return (
    <section className="flex flex-col md:flex-row md:justify-between items-center w-full py-9">
      <div className="md:w-[45%] order-2 md:order-1">
        ...
        <p className="font-inter text-[#2A3337] dark:text-primary-dark xl:text-lg leading-[32px] 2xl:leading-[40px] my-4 dark:text-gray-300">Pagechap is a one-link-for-all solution that allows creators house all their content in one dynamic page.
Join our waitlist and we’d let you know when we’re up and running!</p>
       <form className="space-y-4 md:space-y-0 font-inter md:start md:space-x-4 flex flex-col md:flex-row my-8">
         <input type="email" placeholder="Email Address" className="w-full h-[56px] 2xl:h-[64px] bg-[#F1F1F2] text-[#f1f1f2] 2xl:text-xl dark:bg-[#1D2225] rounded-xl px-6 focus:outline-none"/>
